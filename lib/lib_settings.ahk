...
settingsSectionInit(sectionValue)
{
    ...
    if tempLen is not number ;如果还没初始化过
    {
        ...
        for key,keyValue in keyArr
        {
            ...
            if (sectionValue ~= QSearch|QRun|QWeb)  ;如果是这些里面的，用对象来保存，否则直接key=value
            {
    ...
    {
        _clsetsSec:=CLSets[sectionValue]
        if (sectionValue ~= QSearch|QRun|QWeb)
        {
            for key,value in  _clsetsSec
            {
                _fullKey:=value.fullKey
                valNew := IniRead(CapsLock+settings.ini, %sectionValue%, %_fullKey%, %A_Space%)
                if(valNew="") ;已删除
                {
